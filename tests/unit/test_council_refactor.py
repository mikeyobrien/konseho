...
from konseho.core.steps import Step, DebateStep, StepResult
from konseho.core.context import Context
from konseho.factories import CouncilDependencies, CouncilFactory
from konseho.adapters import MockAgent, MockStep, MockEventEmitter, MockOutputManager
...
class TestErrorHandler:
    ...
    async def test_retry_strategy(self):
        ...
        result = await handler.handle_step_error(error, step, "task", Context(), attempt=0)
        assert result is None
        
        # Second retry - should still return None
        result = await handler.handle_step_error(error, step, "task", Context(), attempt=1)
        assert result is None
...
class TestCouncilRefactored:
    ...
    def test_set_moderator_pool(self):
        ...
        council = Council(
            name="test",
            steps=[DebateStep([agent3, agent4])],
            dependencies=deps
        )
