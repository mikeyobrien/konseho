        ...
        if isinstance(response, list) and all(isinstance(r, dict) for r in response):
            for item in response[:max_results]:
                result = {
                    ...
                    'snippet': str(item.get('snippet', item.get('description', item.get('content', 'No description available'))))
                }
            ...
            if num_match:
                ...
                if url_match:
                    ...
                    if title_part:
                        ...
                        # Or after the URL
                        current_result['title'] = content[url_match.end():].strip(' -') or 'Search Result'
                else:
...
def create_mcp_search_provider(
    ...
    for tool in tools:
        ...
        if ('search' in tool_name or 'search' in tool_doc) and pattern_lower in tool_name:
            return MCPSearchProvider(tool)
