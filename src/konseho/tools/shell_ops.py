...
async def async_terminal_approval_callback(command: str, error_msg: str
    ) ->bool:
...
ALLOWED_COMMANDS = {'ls', 'dir', 'pwd', 'cd', 'cat', 'type', 'echo', 'grep',
    'find', 'python', 'python3', 'pip', 'pip3', 'uv', 'poetry', 'pipenv',
    'git', 'hg', 'svn', 'npm', 'yarn', 'node', 'make', 'cargo', 'go',
    'java', 'javac', 'pytest', 'unittest', 'mypy', 'ruff', 'black',
    'flake8', 'pylint', 'whoami', 'hostname', 'date', 'which', 'where'}
...
def add_allowed_commands(*commands: str) ->None:
    ...
    logger.info(
        f"Added {len(commands)} commands to allowlist: {', '.join(commands)}")
...
def validate_command(command: str) ->tuple[bool, str]:
    ...
    for part in parts[1:]:
        for pattern in dangerous_arg_patterns:
            if pattern in part:
                return (False,
                    f"Dangerous pattern '{pattern}' detected in arguments")
        ...
        if not in_single_quote and not in_double_quote:
            for pattern in dangerous_shell_patterns:
                if command[i:i + len(pattern)] == pattern:
                    if pattern == '&' and i + 1 < len(command) and command[
                        i + 1] == '&':
                        continue
                    return (False,
    ...
    if not allow_unsafe:
        ...
        if not is_valid:
            if approval_callback:
                ...
                if approved := approval_callback(command, error_msg):
                    logger.warning(
                        f'User approved dangerous command: {command}')
                    result = shell_run(command=command, cwd=cwd, timeout=
                        timeout, capture_output=capture_output,
                        allow_unsafe=True, approval_callback=None)
                    result['approved'] = True
                    return result
                else:
                    logger.warning(
                        f'User rejected dangerous command: {command}')
                    return {'error': f'Command rejected: {error_msg}',
                        'returncode': -1, 'stdout': '', 'stderr': '',
                        'approved': False}
            else:
                logger.warning(f'Command validation failed: {error_msg}')
                return {'error': error_msg, 'returncode': -1, 'stdout': '',
                    'stderr': ''}
    ...
    try:
        try:
            ...
            return {'error': f'Failed to parse command: {str(e)}',
                'returncode': -1, 'stdout': '', 'stderr': ''}
        completed = subprocess.run(cmd_parts, cwd=cwd, timeout=timeout,
...
def execute_piped_commands(commands: list[str], cwd: (str | None)=None,
    ...
    """
    if not commands:
        return {'error': 'No commands provided', 'returncode': -1, 'stdout':
            '', 'stderr': ''}
    for cmd in commands:
        is_valid, error_msg = validate_command(cmd)
        if not is_valid:
            return {'error': f'Command validation failed: {error_msg}',
                'returncode': -1, 'stdout': '', 'stderr': ''}
    result = {'returncode': -1, 'stdout': '', 'stderr': ''}
    try:
        ...
        for i, cmd in enumerate(commands):
            ...
            if i == 0:
                ...
                proc = subprocess.Popen(cmd_parts, stdin=processes[-1].
                    stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                    text=True, cwd=cwd)
        ...
        if not is_valid:
            if approval_callback:
                ...
                if asyncio.iscoroutinefunction(approval_callback):
                    ...
                    loop = asyncio.get_event_loop()
                    approved = await loop.run_in_executor(None,
                        approval_callback, command, error_msg)
                if approved:
                    logger.warning(
                        f'User approved dangerous command: {command}')
                    result = await async_shell_run(command=command, cwd=cwd,
                ...
                return {'error': error_msg, 'returncode': -1, 'stdout': '',
                    'stderr': ''}
    loop = asyncio.get_event_loop()
    result = await loop.run_in_executor(None, lambda : shell_run(command=
