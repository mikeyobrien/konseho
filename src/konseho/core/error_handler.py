...
class ErrorHandler:
    ...
    def __init__(
        ...
        max_retries: int = 3,
        fallback_handler: Optional[Callable[[Exception, IStep, str, "Context"], Coroutine[Any, Any, IStepResult]]] = None,
    ...
    async def handle_step_error(
        ...
        if self.event_emitter:
            self.event_emitter.emit("step_error", {
                "step": step.name,
                "error": str(error),
                "attempt": attempt,
        ...
        if self.error_strategy == ErrorStrategy.HALT:
            ...
            if attempt < self.max_retries:
                logger.info(f"Retrying {step.name} (attempt {attempt + 1}/{self.max_retries})")
            ...
            if self.fallback_handler:
                ...
                logger.warning(f"No fallback handler configured, continuing after error")
                return StepResult(
        ...
        while True:
            try:
                ...
                if attempt > 0 and self.event_emitter:
                    self.event_emitter.emit("step_retry_success", {
                        "step": step.name,
                        "attempts": attempt + 1
                    })
                return result
