...
class OutputManager:
    ...
    def save_output(self, task: str, result: dict[str, Any], council_name:
        ...
        output_data = {'task': task, 'council_name': council_name,
    ...
    def save_formatted_output(self, task: str, result: dict[str, Any],
        ...
        if filename is None:
            ...
            task_hash = hashlib.md5(task.encode()).hexdigest()[:8]
            filename = f'{timestamp}_{task_hash}'
        output_lines = ['COUNCIL EXECUTION REPORT', f"{'=' * 80}",
            f'Council: {council_name}', f'Task: {task}',
                                    ...
                                    if isinstance(content, str):
                                        try:
                                            ...
                                            if isinstance(parsed, list) and len(parsed
                                                ) > 0 and isinstance(parsed[0], dict):
                                        ...
                                        text = str(content)
                                    output_lines.append(text[:500] + '...' if
                                        len(text) > 500 else text)
                    else:
        ...
        with open(json_path, 'w', encoding='utf-8') as f:
            json.dump({'task': task, 'council_name': council_name,
                'timestamp': datetime.now().isoformat(), 'result': result,
    ...
    def list_outputs(self, council_name: (str | None)=None) ->list[dict[str,
        ...
        outputs = []
        if council_name:
            search_dirs = [self.base_dir / council_name.replace(' ', '_').
                lower()]
        else:
            ...
            for json_file in council_dir.glob('*.json'):
                try:
                    with open(json_file) as f:
                        data = json.load(f)
                        outputs.append({'file': str(json_file), 'task':
                            data.get('task', ''), 'council_name': data.get(
                            'council_name', ''), 'timestamp': data.get(
                            'timestamp', ''), 'has_text': json_file.
