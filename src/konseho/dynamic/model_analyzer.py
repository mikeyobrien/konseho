...
class ModelBasedAnalyzer:
    ...
    def __init__(self, model: (str | None)=None, temperature: float=0.3):
        ...
        prompt_with_registry = ANALYZER_PROMPT.replace('{persona_registry}',
            self.registry.get_registry_summary())
        self._analyzer_agent = create_agent(name='QueryAnalyzer',
    ...
    async def analyze(self, query: str) ->dict[str, JSON]:
        ...
        try:
            ...
            agent_result = await loop.run_in_executor(None, self.
                _analyzer_agent, query)
            json_str = None
            analysis = None
            if result_str := str(agent_result):
            ...
            if analysis is None:
                raise ValueError(
                    'Failed to extract analysis from model response')
            task_type_str = analysis.get('task_type', 'general')
            analysis['task_type'] = self._parse_task_type(task_type_str)
            analysis['suggested_agent_count'] = len(analysis.get(
                'suggested_agents', []))
            analysis['needs_parallel'] = any(step['type'] == 'parallel' for
    ...
    def _parse_task_type(self, task_type_str: str) ->TaskType:
        ...
        mapping = {'research': TaskType.RESEARCH, 'code_review': TaskType.
            CODE_REVIEW, 'design': TaskType.DESIGN, 'analysis': TaskType.
            ANALYSIS, 'planning': TaskType.PLANNING, 'debate': TaskType.
            DEBATE, 'implementation': TaskType.IMPLEMENTATION, 'general':
            TaskType.GENERAL}
