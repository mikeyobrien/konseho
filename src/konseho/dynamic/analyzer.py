...
class QueryAnalyzer:
    ...
    def __init__(self):
        self.task_patterns = {
            TaskType.RESEARCH: [
                r"\bresearch\b", r"\bfind out\b", r"\bexplore\b", r"\binvestigate\b",
                r"\bgather information\b", r"\blearn about\b", r"\bstudy\b"
            ],
            TaskType.CODE_REVIEW: [
                r"\breview\b.*\bcode\b", r"\bcode review\b", r"\bcheck.*\bcode\b",
                r"\baudit\b", r"\bfind.*\bbug\b", r"\banalyze.*\bcode\b"
            ],
            TaskType.DESIGN: [
                r"\bdesign\b", r"\barchitect\b", r"\bcreate.*\bsystem\b",
                r"\bpropose.*\bsolution\b", r"\bbuild\b", r"\bimplement\b"
            ],
            TaskType.ANALYSIS: [
                r"\banalyze\b", r"\bevaluate\b", r"\bassess\b", r"\bcompare\b",
                r"\bexamine\b", r"\binspect\b", r"\bstudy\b"
            ],
            TaskType.PLANNING: [
                r"\bplan\b", r"\bstrategy\b", r"\broadmap\b", r"\borganize\b",
                r"\bschedule\b", r"\bcoordinate\b"
            ],
            TaskType.DEBATE: [
                r"\bdebate\b", r"\bdiscuss\b", r"\bpros.*\bcons\b", r"\bargue\b",
                r"\bcontrast\b", r"\bcompare.*\bapproaches\b"
            ],
            TaskType.IMPLEMENTATION: [
                r"\bimplement\b", r"\bcode\b", r"\bbuild\b", r"\bcreate\b",
                r"\bdevelop\b", r"\bwrite.*\bcode\b"
            ]
        }
        
        self.domain_keywords = {
            "technical": ["code", "api", "database", "system", "architecture", "algorithm"],
    ...
    def _estimate_complexity(self, query: str) -> str:
        ...
        has_multiple_parts = any(word in query.lower() for word in ["and", "also", "then", "multiple", "various"])
        has_constraints = any(word in query.lower() for word in ["must", "should", "require", "constraint", "ensure"])
