...
class ModelStepPlanner:
    ...
    def create_steps_from_spec(
        self, workflow_specs: list[dict[str, Any]], agents: list[AgentWrapper]
    ) -> list[Step]:
        ...
        """
        # Create agent name to instance mapping
        ...
        for spec in workflow_specs:
            ...
            # Get agent instances for participants
            step_agents = [
                agent_map[name] for name in participants if name in agent_map
            ]

            # If no specific participants, use all agents
            if not step_agents:
                step_agents = agents
            ...
            if step_type == "debate":
                step = DebateStep(
                    agents=step_agents,
                    rounds=self._determine_rounds(description),
                    voting_strategy=self._determine_voting_strategy(len(step_agents)),
                )
            elif step_type == "parallel":
                step = ParallelStep(
                    agents=step_agents,
                    task_splitter=self._create_task_splitter(description, participants),
                )
            elif step_type == "split":
                step = SplitStep(
                    agent_template=(
                        step_agents[0].agent if step_agents else agents[0].agent
                    ),
    ...
    def _create_task_splitter(
        self, description: str, participants: list[str]
    ) -> callable:
            ...
            if len(participants) == n:
                ...
                for participant in participants[:n]:
                    if "review" in description.lower():
                        tasks.append(f"As {participant}, review and analyze: {task}")
