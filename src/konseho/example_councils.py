...
def create_innovation_council() -> Council:
    ...
    explorer = AgentWrapper(
        create_agent(name="Explorer", system_prompt=EXPLORER_PROMPT, temperature=0.9),
        name="Explorer",
    )

    visionary = AgentWrapper(
        create_agent(name="Visionary", system_prompt=VISIONARY_PROMPT, temperature=0.9),
        name="Visionary",
    )

    critic = AgentWrapper(
        create_agent(name="Critic", system_prompt=CRITIC_PROMPT, temperature=0.6),
        name="Critic",
    )
...
def create_development_council() -> Council:
    ...
    coder = AgentWrapper(
        create_agent(name="Coder", system_prompt=CODER_PROMPT, temperature=0.5),
        name="Coder",
        expertise_level=1.0,
    )

    analyst = AgentWrapper(
        create_agent(name="Analyst", system_prompt=ANALYST_PROMPT, temperature=0.6),
        name="Analyst",
        expertise_level=0.9,
    )
    ...
    return Council(
        ...
        steps=[
            ...
            DebateStep(
                agents=[planner, analyst], rounds=1, voting_strategy="consensus"
            ),
...
def create_research_council() -> Council:
    ...
    critic = AgentWrapper(
        create_agent(name="Critic", system_prompt=CRITIC_PROMPT, temperature=0.7),
        name="Critic",
    )
