...
class StrandsMCPManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if not MCP_AVAILABLE:
            logger.warning("MCP support not available. Install 'mcp' package to enable MCP tools.")
            self.clients = {}
    ...
    def get_search_tool(self, server_name: str = None) -> Optional[Callable]:
        ...
        if server_name:
            ...
            logger.info(f"Looking for search tool in {len(tools)} tools from {server_name}")
            for tool in tools:
                # Debug: log tool info
                ...
                if hasattr(tool, '__name__'):
                    if 'search' in tool.__name__.lower() or 'brave' in tool.__name__.lower():
